        ...
        setPending(session, {
          fn: async () => {
            ...
            console.log(`[bot] list-fallback intent=${reparsed.intent}`,
                        JSON.stringify(reparsed.data));
            return dispatchIntent(reparsed.intent, reparsed.data, session, reparsed.reply);
          }
        });
...
- Convert relative dates ("tomorrow", "next Friday", "in 3 days", "Tuesday 2nd")
  ...
  future date that matches the user's wording. Never leave a date blank.
  ...
  if (intent === 'add_event') {
    ...
    if (Array.isArray(data.events) && data.events.length) {
      ...
      for (const e of data.events) {
        if (!e || !e.title || typeof e.title !== 'string') {
          errors.push(`• (missing title)`);
          continue;
        }
        if (!isValidYMD(e.date)) {
          errors.push(`• "${e.title}" — couldn't read the date`);
          continue;
        }
        if (!isValidHM(e.time)) {
          errors.push(`• "${e.title}" — bad time format`);
          continue;
        }
        const ev = {
          id:    newId(),
      ...
      if (!added.length) {
        return `❌ I couldn't read any of those events.` +
               (errors.length ? `\n${errors.join('\n')}` : '');
      }
      ...
      session.lastAction = {
        label: `Added ${added.length} events`,
        undo: () => {
          const cur = get('calendar').events || [];
          set('calendar', { events: cur.filter(x => !ids.includes(x.id)) });
          return `Removed ${added.length} event${added.length !== 1 ? 's' : ''} (undo).`;
        }
      };

      const lines = added.map(a =>
        `• *${a.title}* — ${fmtDate(a.date)}${a.time ? ` ⏰ ${a.time}` : ''}`
      ).join('\n');

      let out = `📅 *Added ${added.length} event${added.length !== 1 ? 's' : ''} to calendar:*\n\n${lines}`;
